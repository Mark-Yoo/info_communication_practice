...
const addTodo = ({ keyCode }) => {
  ...
  uniFetch.post({id: generateId(), content, completed: false})
    .then(data => todos = data)
    .then(render)
  $inputTodo.value = '';
}
...
const toggleTodo = ({ target }) => {
  ...
  uniFetch.patch(id, {completed: target.checked})
    .then(data => todos = data)
    .then(render);
}
...
const toggleAll = ({ target }) => {
  uniFetch.patchAll({completed: target.checked})
    .then(data => todos = data)
    .then(render)
};
